...
export const Creacion = ({ d, set, onSave, saving, saved, isEditing }: Props) => {
  const { clientes, proyectos, herramientas } = useWizardCatalogos();

  const [selectedClienteId, setSelectedClienteId] = useState<string>(() => {
    if (!d.proyecto_id) return '';
    return proyectos.find(p => p.id === d.proyecto_id)?.cliente_id ?? '';
  });

  const proyectosFiltrados = useMemo(
    () => selectedClienteId ? proyectos.filter(p => p.cliente_id === selectedClienteId) : [],
    [selectedClienteId, proyectos],
  );
  ...
  const handleHerramientaToggle = (id: string) => {
    const current = (d.herramientas_ids as string[]) ?? [];
    set('herramientas_ids', current.includes(id) ? current.filter(h => h !== id) : [...current, id]);
  };
  ...
  return (
    <div className="step-body">
      ...
      <div className="wfield">
        ...
        <select className="wfield__input" value={selectedClienteId} onChange={e => handleClienteChange(e.target.value)}>
        ...
        {herramientas.length === 0 ? (
          ...
          <div className="wfield__checkbox-group">
            {herramientas.map(h => {
              ...
              return (
                <label key={h.id} className={`wfield__checkbox-item ${selected ? 'wfield__checkbox-item--selected' : ''}`}>
                  <input type="checkbox" checked={selected} onChange={() => handleHerramientaToggle(h.id)} />
                  <span>{h.nombre}</span>
                </label>
              );
            })}
